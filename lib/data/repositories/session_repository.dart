...
class SessionRepository {
  ...
  Future<WorkSession> startSession(int employeeId, {DateTime? at}) async {
    ...
    final existing = await (_db.select(_db.workSessions)
          ..where((s) =>
              s.employeeId.equals(employeeId) & s.endedAt.isNull()))
        .getSingleOrNull();
    if (existing != null) {
  ...
  Future<void> endSession(int sessionId, {DateTime? at}) async {
    ...
    await _db.transaction(() async {
      final openBreak = await (_db.select(_db.breaks)
            ..where((b) =>
                b.sessionId.equals(sessionId) & b.endedAt.isNull()))
          .getSingleOrNull();
      if (openBreak != null) {
  ...
  Future<Break> startBreak(int sessionId, {DateTime? at}) async {
    final now = at ?? DateTime.now().toUtc();
    final existing = await (_db.select(_db.breaks)
          ..where((b) =>
              b.sessionId.equals(sessionId) & b.endedAt.isNull()))
        .getSingleOrNull();
    if (existing != null) {
    ...
    final id = await _db.into(_db.breaks).insert(
          BreaksCompanion.insert(sessionId: sessionId, startedAt: now),
        );
    return (_db.select(_db.breaks)..where((b) => b.id.equals(id)))
        .getSingle();
  }

  Future<void> endBreak(int breakId, {DateTime? at}) async {
    final now = at ?? DateTime.now().toUtc();
    await (_db.update(_db.breaks)..where((b) => b.id.equals(breakId)))
        .write(BreaksCompanion(endedAt: Value(now)));
  }
